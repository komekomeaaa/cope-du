      ...
      <main id="main-content">
            ...
            <div className="grid md:grid-cols-2 max-w-5xl mx-auto gap-8">
              {prioritizedProducts.map((product, index) => {
                ...
                return (
                  <Card
                    key={product.id}
                    className={`group relative bg-white border border-slate-200/60 shadow-lg hover:shadow-xl hover:border-cyan-300 transition-[opacity,transform,box-shadow,border-color] duration-500 overflow-hidden ${
                      product.popular ? 'ring-1 ring-cyan-500/50' : ''
                    } ${
                  ...
                  >
                    {product.popular && (
                      <div className="absolute top-0 right-0 bg-cyan-600 text-white text-xs px-4 py-1 rounded-bl-lg font-medium">
                        人気No.1
                      </div>
                    )}
                    ...
                    <CardHeader className="p-8 text-center">
                      ...
                      <CardTitle className="text-2xl font-normal text-slate-900 mb-2 font-[family-name:var(--font-display)]">
                        {product.name}
                      </CardTitle>
                      <p className="text-cyan-600 font-medium mb-4">{product.tagline}</p>
                    ...
                    <CardContent className="px-8 pb-8">
                      <div className="space-y-3">
                        {product.features.map((feature, idx) => (
                          <div key={idx} className="flex items-center gap-2 text-sm text-slate-600">
                            <Check aria-hidden="true" className="h-4 w-4 text-cyan-600 flex-shrink-0" />
                            <span>{feature}</span>
                          </div>
                        ))}
                      </div>
                      <div className="mt-8 text-center">
                        <Link
                          ...
                          className="inline-flex items-center justify-center px-4 py-2 rounded-md bg-gradient-to-r from-cyan-600 to-blue-600 text-white hover:from-cyan-500 hover:to-blue-500 transition-all shadow-lg shadow-cyan-600/20"
                        >
                          詳細を見る
                        </Link>
                      </div>
        ...
        <section className="py-20 px-4">
          <div className="max-w-6xl mx-auto">
            ...
            >
              <h2 className="text-4xl md:text-5xl font-normal text-slate-900 mb-4 text-wrap-balance font-[family-name:var(--font-display)]">
                AIソリューションの特長
              </h2>
            ...
            <div className="grid md:grid-cols-3 gap-12">
              {saasFeatures.map((feature, index) => {
                ...
                return (
                  ...
                  >
                    <div className="w-14 h-14 mx-auto mb-6 bg-cyan-50 rounded-xl flex items-center justify-center">
                      {IconComponent && <IconComponent aria-hidden="true" className="h-7 w-7 text-cyan-600" />}
                    </div>
                    <h3 className="text-2xl font-normal text-slate-900 mb-4 font-[family-name:var(--font-display)]">{feature.title}</h3>
